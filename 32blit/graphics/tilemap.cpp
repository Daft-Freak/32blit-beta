...
namespace blit {
  ...
  TileMap *TileMap::load_tmx(const uint8_t *asset, Surface *sprites, int layer, int flags) {
    ...
    auto layer_base = map_struct->data + layer_size * map_struct->layers;

    uint8_t *transform_data;
    if(flags & copy_transforms) {
      transform_data = new uint8_t[layer_size];
      memcpy(transform_data, layer_base + layer_size * layer, layer_size);
    } else {
      transform_data = const_cast<uint8_t *>(layer_base + layer_size * layer);
    }
