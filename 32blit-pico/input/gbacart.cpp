...
void init_gbacart_input() {
}

void update_gbacart_input(uint32_t &new_buttons, blit::Vec2 &new_joystick) {
  ...
  new_buttons = new_buttons
              | ((button_state & (1 << 5)) ? uint32_t(Button::DPAD_LEFT) : 0)
...
extern const InputDriver gbacart_input_driver {
  init_gbacart_input, update_gbacart_input
};
